...
export async function resolveGardenParams(currentDirectory: string, opts: GardenOpts): Promise<GardenParams> {
  ...
  if (!opts.noEnterprise && cloudApi) {
    ...
    try {
      secrets = await getSecrets({ log: enterpriseLog, environmentName, cloudApi })
      enterpriseLog.setSuccess({ msg: chalk.green("Ready"), append: true })
      enterpriseLog.silly(`Fetched ${Object.keys(secrets).length} secrets from ${cloudApi.domain}`)
    } catch (err) {
  ...
  const enterpriseDomain = cloudApi?.domain
